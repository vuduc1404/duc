...
async def generate_edge_tts_audio(sentence, speaker_id, output_path, rate=1.0):
    try:
        ...
        # Đảm bảo có dấu "+" hoặc "-" phía trước
        if percent >= 0:
            rate_str = f"+{percent}%"
        else:
            rate_str = f"{percent}%"
        ...
        # Kiểm tra file audio thực sự tồn tại và hợp lệ
...
async def render_sentence(
    ...
    async with sem:
        ...
        if is_video_input:
            ...
            cmd = [
                norm_ffmpeg_path, '-y', '-i', norm_video_path,
                '-i', norm_audio_path, '-i', norm_sub_path,
                '-filter_complex',
                f"[0:v]{vf_chain}[vbg];[vbg][2:v]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)-30:enable='between(t,0,{duration:.2f})'[v];[1:a]volume={volume_factor}[a]",
                '-map', '[v]', '-map', '[a]',
                '-c:v', encoder, '-r', '25',
            ] + encoder_preset_option + [
                '-threads', str(os.cpu_count()), '-shortest', '-an',
                norm_temp_out
            ]
...
async def render_shard(
    ...
    offset_in_all=0, voice_source="Voicevox"
):
    ...
    # Sửa: chỉ số câu toàn cục dựa trên offset_in_all truyền từ ngoài vào
