...
async def render_sentence(
    ...
    effects_dir=None # bổ sung tham số này
):
    async with sem:
        ...
        if is_video_input:
            ...
            map_video = "[v]"
            overlay_cmd = []
            if effect in ["snow", "sakura"] and effects_dir is not None:
                overlay_mov = os.path.join(effects_dir, f"{effect}_alpha.mov")
                if os.path.exists(overlay_mov):
                    overlay_mov_ffmpeg = normalize_path_for_ffmpeg(overlay_mov)
                    inputs.append(overlay_mov_ffmpeg)
                    # [0:v]=gốc, [2:v]=sub, [3:v]=overlay
                    filter_complex = (
                        f"[0:v]{vf_chain}[vbg];"
